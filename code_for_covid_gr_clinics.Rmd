...
covid_gr_clinics <- dplyr::rename(covid_gr_clinics, DL_BEH_B = DAILY_BEHAVIORS_BASE_KI_SUM)
covid_gr_clinics <- dplyr::rename(covid_gr_clinics, DL_BEH_3m = DAILY_BEHAVIORS_3M_KI_SUM)
```
...
#covid_gr_clinics_train_long<- covid_gr_clinics_train %>% 
 # gather (key, val, c(MD_ST_B, MD_ST_3m, DL_BEH_B, DL_BEH_3m))
```

behavior_score, DL_BEH_B, DL_BEH_3m
...
subsample_covid_gr_clinics_train_long  <- covid_gr_clinics_train_long[covid_gr_clinics_train_long$id %in%  sample(unique(covid_gr_clinics_train$id), 50),] # this is the sampling command
...
```{r}
ids_for_plots <- sample(unique(covid_gr_clinics_train_long$id), 80, replace = FALSE, prob = NULL)
subsample_for_plots <- subsample_covid_gr_clinics_train_long %>% 
  filter(id %in% ids_for_plots)
```
...
plot(bootstrap_diff, rawplot.ylim = c(0, 40), effsize.ylim = c(-10,10))
```
